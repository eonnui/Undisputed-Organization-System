...
function applyUserTheme() {
  ...
  fetch(`/get_user_data${themeQueryParam}`)
    ...
    .then((data) => {
      ...
      const organizationNameDisplay = document.getElementById(
    ...
    .catch(() => {
      ...
      if (profilePicElement)
        profilePicElement.src = "/static/images/your_image_name.jpg";
      if (organizationNameDisplay)
        organizationNameDisplay.textContent = "Organization System";
    });
}
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        notificationsDropdown.innerHTML =
          '<div class="notification-item">Please log in to see notifications.</div>';
        return [];
      }
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    ...
    notifications.forEach((notification) => {
      ...
      clearButton.addEventListener("click", async (event) => {
        ...
        const notificationIdsToClear = notification.group_ids || [
          notification.id,
        ];
        let successCount = 0;
    ...
    notificationsDropdown.innerHTML =
      '<div class="notification-item">Failed to load notifications.</div>';
    return [];
  }
}
...
async function clearAllNotifications() {
  try {
    const response = await fetch(`/notifications/clear_all`, {
      method: "DELETE",
    });
    return response.ok;
  } catch (error) {
...
async function updateBadgeBasedOnNewUnread() {
  ...
  try {
    const currentUnreadNotifications = await fetchAndDisplayNotifications(
      false
    );
    const currentUnreadIds = new Set();
    currentUnreadNotifications.forEach((notification) => {
      (notification.group_ids || [notification.id]).forEach((id) =>
        currentUnreadIds.add(id)
      );
    });
        ...
        fetchAndDisplayNotifications(true).then((notifications) => {
          ...
          notifications.forEach((notification) => {
            if (!notification.is_read) {
              (notification.group_ids || [notification.id]).forEach((id) =>
                lastSeenUnreadNotificationIds.add(id)
              );
            }
          });

          localStorage.setItem(
            "lastSeenUnreadNotificationIds",
            JSON.stringify(Array.from(lastSeenUnreadNotificationIds))
          );
        });
        clearInterval(notificationPollingTimer);
      } else {
...
function initializeSidebarToggle() {
  ...
  menuToggle.addEventListener("click", () => {
    const isCurrentlyCollapsed = document
  ...
  window.addEventListener("click", function (event) {
    document
      ...
      .forEach((openDropdown) => {
        ...
        if (
          relatedButton &&
          !relatedButton.contains(event.target) &&
