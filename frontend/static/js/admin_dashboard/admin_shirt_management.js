document.addEventListener("DOMContentLoaded", function () {
  ...
  const modalContent = globalModal
    ? globalModal.querySelector(".modal-content")
    : null;
  const modalBody = globalModal ? document.getElementById("modal-body") : null;
  const closeModalBtn = globalModal
  ...
  if (!closeModalBtn) {
    console.warn(
      "admin_shirt_management.js: Close button (.modal-close-btn) not found for global modal."
    );
  }
  ...
  function openGlobalModal(title, contentHtml) {
    ...
    if (!modalBody) {
      console.error(
        "openGlobalModal: Necessary modal element (#modal-body) not found. Cannot set content."
      );
      return;
    }
    ...
    modalBody.innerHTML = `
        ...
        `;
    console.log(
      "openGlobalModal: Modal title and content HTML set in modalBody."
    );
    ...
    if (globalModal) {
      ...
      console.error(
        "openGlobalModal: globalModal not found, cannot add 'is-visible' class."
      );
    }
  }
  ...
  function closeGlobalModal() {
    ...
    if (globalModal) {
      ...
      console.log(
        "closeGlobalModal: 'is-visible' class removed from globalModal."
      );
    } else {
      console.error(
        "closeGlobalModal: globalModal not found, cannot remove 'is-visible' class."
      );
    }
  ...
  if (closeModalBtn) {
    ...
    console.log(
      "admin_shirt_management.js: Close button event listener attached."
    );
  }
  ...
  if (globalModal) {
    ...
    console.log(
      "admin_shirt_management.js: Global modal overlay click listener attached."
    );
  }
  ...
  if (modalBody) {
    modalBody.addEventListener("click", function (event) {
      if (event.target.id === "addCreateCampaignSizePrice") {
        ...
        if (container) {
          ...
          console.error(
            "createCampaignSizePricesContainer not found when adding row."
          );
        }
        ...
        if (container) {
          ...
          console.error(
            "editCampaignSizePricesContainer not found when adding row."
          );
        }
    ...
    console.error(
      "admin_shirt_management.js: modalBody not found, cannot attach size/price row event listener."
    );
  }
  ...
  function collectSizePriceData(containerId) {
    ...
    if (!container) return pricesBySize;
  ...
  async function handleCreateCampaign(event) {
    ...
    const preOrderDeadlineInput = form.querySelector("#preOrderDeadline");
    const availableStockInput = form.querySelector("#campaignStock");

    const preOrderDeadline = new Date(preOrderDeadlineInput.value);
    const availableStock = parseInt(availableStockInput.value);
    const now = new Date();

    if (preOrderDeadline <= now) {
      showAlert("Pre-Order Deadline cannot be in the past.", "danger");
      return;
    }

    if (availableStock < 0) {
      showAlert("Available Stock cannot be negative.", "danger");
      return;
    }
    ...
    try {
      ...
      if (createCampaignSizePricesContainer) {
        createCampaignSizePricesContainer.innerHTML = "";
        createCampaignSizePricesContainer.appendChild(createSizePriceRow());
      } else {
        console.error(
          "createCampaignSizePricesContainer not found for resetting."
        );
      }
  ...
  async function handleEditCampaign(event) {
    ...
    const editPreOrderDeadlineInput = form.querySelector(
      "#editCampaignPreOrderDeadline"
    );
    const editAvailableStockInput = form.querySelector("#editCampaignStock");

    const editPreOrderDeadline = new Date(editPreOrderDeadlineInput.value);
    const editAvailableStock = parseInt(editAvailableStockInput.value);
    const now = new Date();

    if (editPreOrderDeadline <= now) {
      showAlert("Pre-Order Deadline cannot be in the past.", "danger");
      return;
    }

    if (editAvailableStock < 0) {
      showAlert("Available Stock cannot be negative.", "danger");
      return;
    }
                    ...
                    <td>
                        ${
                          campaign.size_chart_image_path
                            ? `<img src="${campaign.size_chart_image_path}" alt="Size Chart" style="max-width: 80px; height: auto; border-radius: 8px;">`
                            : "No Image"
                        }
                    </td>
                    <td>
  ...
  const editCampaignFormTemplate = (campaign) => {
    ...
    if (
      ...
      for (const size in campaign.prices_by_size) {
        sizePricesHtml += createSizePriceRow(
          size,
          campaign.prices_by_size[size]
        ).outerHTML;
      }
    } else {
    ...
    return `
            <form id="editCampaignForm" enctype="multipart/form-data" style="color: var(--org-text-primary);">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="editCampaignTitle" name="title" value="${
                      campaign.title
                    }" required>
                </div>
                <div class="mb-3">
                    <label for="editCampaignDescription" class="form-label">Description</label>
                    <textarea class="form-control" id="editCampaignDescription" name="description" rows="3">${
                ...
                <div class="mb-3">
                    ...
                    <input type="datetime-local" class="form-control" id="editCampaignPreOrderDeadline" name="pre_order_deadline" value="${formatDateForInput(
                      campaign.pre_order_deadline
                    )}" required>
                </div>
                <div class="mb-3">
                    <label for="editCampaignStock" class="form-label">Available Stock</label>
                    <input type="number" class="form-control" id="editCampaignStock" name="available_stock" value="${
                ...
                <div class="mb-3">
                    ...
                    <div id="currentSizeChartImage" class="mt-2">
                        ${
                          campaign.size_chart_image_path
                            ? `<img src="${campaign.size_chart_image_path}" style="max-width: 150px; height: auto; border-radius: 8px;"><p>Current Image</p>`
  ...
  const openCreateCampaignModalBtn = document.getElementById(
    "openCreateCampaignModalBtn"
  );
  if (openCreateCampaignModalBtn) {
    console.log(
      "admin_shirt_management.js: openCreateCampaignModalBtn found, attaching listener."
    );
    openCreateCampaignModalBtn.addEventListener("click", function () {
      ...
      if (cancelBtn) {
        ...
        console.warn(
          "cancelCreateCampaign button not found after opening modal."
        );
      }
    });
  } else {
    console.error(
      "admin_shirt_management.js: openCreateCampaignModalBtn NOT found! Check HTML ID."
    );
  }
  ...
  if (campaignsTableBody) {
    console.log(
      "admin_shirt_management.js: campaignsTable tbody found, attaching delegated listener."
    );
    campaignsTableBody.addEventListener("click", async function (event) {
      ...
      if (event.target.classList.contains("edit-campaign-btn")) {
        ...
        try {
          ...
          openGlobalModal(
            "Edit Shirt Campaign",
            editCampaignFormTemplate(campaign)
          );
          ...
          if (cancelBtn) {
            ...
            console.warn(
              "cancelEditCampaign button not found after opening modal."
            );
          }
        } catch (error) {
    ...
    console.error(
      "admin_shirt_management.js: campaignsTable tbody NOT found! Check HTML structure."
    );
  }
  ...
  if (applyOrderFiltersBtn) {
    ...
    console.error(
      "admin_shirt_management.js: applyOrderFilters button NOT found!"
    );
  }

  console.log(
    "admin_shirt_management.js: Calling fetchCampaigns and fetchOrders..."
  );
  fetchCampaigns();
  fetchOrders("");
});
